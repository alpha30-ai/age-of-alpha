...
export default async function EditVideoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-4xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div>
        <h1 className="text-3xl font-bold text-white mb-2">تعديل الفيديو</h1>
        <p className="text-gray-400">تعديل بيانات الفيديو: {video.title}</p>
      </div>

      <form action={updateVideo} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <FileUploadInput name="videoUrl" label="ملف الفيديو (ارفع من جهازك أو رابط خارجي)" defaultValue={video.videoUrl} accept="video/*" />
          <FileUploadInput name="thumbnail" label="صورة الغلاف (Thumbnail) - اختياري" defaultValue={video.thumbnail || ''} accept="image/*" />
        </div>
