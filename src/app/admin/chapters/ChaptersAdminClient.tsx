...
export default function ChaptersAdminClient({ initialChapters, novels }: { initialChapters: any[], novels?: any[] }) {
  ...
  const filteredChapters = initialChapters.filter(ch => {
    const matchesSearch = ch.title.toLowerCase().includes(searchQuery.toLowerCase()) || ch.chapterNum.toString() === searchQuery;
    const matchesNovel = selectedNovelId === 'all' || ch.novelId === selectedNovelId;
