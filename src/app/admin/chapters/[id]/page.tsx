...
export default async function EditChapterPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-4xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div>
        <h1 className="text-3xl font-bold text-white mb-2">تعديل المخطوطة</h1>
        <p className="text-gray-400">تعديل أحداث الفصل {chapter.chapterNum}</p>
      </div>

      <form action={updateChapter} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl space-y-6">
        ...
        <div className="space-y-2">
          <label htmlFor="content" className="block text-sm font-bold text-gray-300">المحتوى الكامل</label>
          <textarea 
            id="content" 
            name="content" 
            required
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <FileUploadInput name="imageUrl" label="صورة البانر (الغلاف)" defaultValue={chapter.imageUrl || ''} accept="image/*" />
          <FileUploadInput name="audioUrl" label="رابط القراءة الصوتية" defaultValue={chapter.audioUrl || ''} accept="audio/*" />
        </div>
