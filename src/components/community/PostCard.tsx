...
export default function PostCard({ post, currentUser, onDeleted }: PostCardProps) {
  ...
  return (
    <motion.div 
      ...
      className="bg-[#111]/80 backdrop-blur-xl border border-white/5 rounded-3xl p-6 shadow-xl relative group hover:border-white/10 transition-colors"
      dir="rtl"
    >
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-white/10 bg-black">
            {post.user.image ? (
          ...
          <div>
            <div className="flex items-center gap-2">
              <h4 className="font-bold text-white font-cairo">{post.user.name || 'مستخدم مجهول'}</h4>
      ...
      <div className="text-gray-300 font-tajawal text-lg leading-relaxed whitespace-pre-wrap mb-4">
        {post.content}
      </div>

      {post.imageUrl && (
        <div className="mt-4 rounded-2xl overflow-hidden border border-white/10 max-h-[500px]">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={post.imageUrl} alt="مرفق المنشور" className="w-full h-full object-cover" loading="lazy" />
        </div>
      )}

      {post.isReported && isAdmin && (
        <div className="mt-4 flex items-center gap-2 bg-red-500/10 text-red-400 px-3 py-2 rounded-lg border border-red-500/20 text-sm font-bold">
