...
export default function VideoCommentsSection({ videoId }: { videoId: string }) {
  ...
  return (
    <div className="mt-16 pt-10 border-t border-white/10 font-tajawal">
      <div className="flex items-center gap-3 mb-8">
        <MessageSquare className="w-8 h-8 text-[var(--theme-primary)]" />
        <h3 className="font-amiri font-bold text-3xl md:text-4xl text-white">النقاشات</h3>
      </div>
      ...
      {session ? (
        ...
        <div className="mb-12 p-6 bg-white/5 border border-white/10 rounded-2xl text-center backdrop-blur-md">
          <p className="text-gray-400 mb-4 font-bold">يجب عليك الانضمام للملحمة لترك تعليق.</p>
          <a href="/login" className="inline-block px-8 py-3 bg-[var(--theme-primary)] hover:bg-[var(--theme-primary)]/80 text-white font-bold rounded-xl transition-all shadow-[0_0_15px_color-mix(in_srgb,var(--theme-primary)_40%,transparent)]">
            تسجيل الدخول
          </a>
        </div>
      )}
